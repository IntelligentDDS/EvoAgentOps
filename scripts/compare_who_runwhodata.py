...
def load_who_data(file_path: Path, is_handcrafted: bool = False) -> dict:
    """Load WHO baseline format data directly (Algorithm-Generated/Hand-Crafted)"""
    ...
    steps = [{"agent_name": e.get(index_agent, "Unknown"), "agent": e.get("content", "")} for e in history]

    # mistake_step is 0-indexed in WHO format, convert to 1-indexed
    mistake_step_str = str(data.get("mistake_step", "0"))
    mistake_step = int(mistake_step_str) + 1 if mistake_step_str.isdigit() else 1

    return {
        "steps": steps,
        "question": data.get("question", ""),
        "original_correct_answer": data.get("ground_truth", ""),
        "root_cause_step": mistake_step,
        "root_cause_agent": data.get("mistake_agent", ""),
    }


async def run_single_who_case(
    method: str,
    file_path: Path,
    output_dir: Path,
    ...
    """Run single WHO format case. Returns result dict."""
    ...
    data = load_who_data(file_path, is_handcrafted)
    case_id = file_path.stem

    logger.info(
        f"[{method}] {case_id}: steps={len(data['steps'])}, gt_step={data['root_cause_step']}, gt_agent={data['root_cause_agent']}"
    )
    ...
    output = {
        ...
        "ground_truth_agent": data["root_cause_agent"],
        "ground_truth_step": data["root_cause_step"],
    ...
    logger.info(
        f"[{method}] {case_id}: pred={result['predicted_step']}, agent_ok={output['is_agent_correct']}, step_ok={output['is_step_correct']}"
    )
    return output
